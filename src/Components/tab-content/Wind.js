...
class Wind extends Component{
    ...
    _showSpeed(speed){
        const block = this.refs.speedBlock;
    ...
    _showWindDirection(degree){
        const arrow = this.refs.arrow;

        arrow.style.transform = `rotate(${135 + degree}deg)`;
    }

    componentDidUpdate(nextProps, nextState){
        if( nextProps.weather.name !== this.props.weather.name ){
            const speed = this.props.weather.wind.speed;
            const deg = this.props.weather.wind.deg;

            this._showSpeed( speed );
            setTimeout( ()=> this._showWindDirection( deg ), 0);
        }
    }
	...
	"/>
                ...
                <span className="speed" ref="speedBlock">0</span>
            </li>
        )
    }
    componentDidMount(){
